  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-400">Held with Derayah&apos;s partner bank (ANB). Fund via SARIE/IBAN transfer or mada.</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          <div className="text-xs text-zinc-400">Reserved (pending orders)</div>
            ...
            <tbody>
              {wallet.transactions.map((tx) => (
                <tr key={tx.id} className="border-b border-zinc-800">
                  <td className="py-2 text-zinc-400">{tx.createdAt.toLocaleString("en-US")}</td>
                  <td className="py-2">{tx.type.replace("_", " ")}</td>
                  <td className="py-2">{tx.description}</td>
                  <td className={`py-2 text-right ${tx.amountHalalas < 0n ? "text-red-400" : "text-emerald-400"}`}>
                    {tx.amountHalalas < 0n ? "-" : "+"}
