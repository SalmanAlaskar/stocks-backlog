  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className={`text-xs px-2 py-1 rounded-full ${marketOpen ? "bg-emerald-500/10 text-emerald-400" : "bg-zinc-800 text-zinc-400"}`}>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
        <div className="text-xs text-zinc-400">Total assets (all accounts)</div>
        <div className="text-4xl font-semibold mt-1">{formatSar(totalAssetsHalalas)}</div>
        <div className={`text-sm mt-1 ${totalGainHalalas < 0n ? "text-red-400" : "text-emerald-400"}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {buckets.map((b) => (
          <Link key={b.key} href={b.href} className="block bg-zinc-900 border border-zinc-800 rounded-xl p-4 hover:border-emerald-500/40">
            ...
            {b.exists || b.key === "wallet" ? (
              <>
                ...
                {b.gainHalalas != null ? (
                  <div className={`text-xs mt-1 ${b.gainHalalas < 0n ? "text-red-400" : "text-emerald-400"}`}>
                    {formatSar(b.gainHalalas)} · {b.gainLabel}
                  </div>
                ) : (
                  <div className="text-xs text-zinc-500 mt-1">{b.gainLabel}</div>
                )}
              </>
            ) : (
              <div className="text-sm text-zinc-500 mt-1">Not set up yet &rarr;</div>
            )}
          </Link>
        ))}
      </div>
      ...
      {goal && (
        <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
          ...
          <div className="flex items-baseline gap-2 mb-2">
            <span className={`text-2xl font-semibold ${goal.returnPct < 0 ? "text-red-400" : "text-emerald-400"}`}>
              {formatPercent(goal.returnPct)}
            </span>
            <span className="text-sm text-zinc-500">of {goal.targetPct}% target by Dec 31, 2026</span>
          </div>
          ...
          <dl className="text-sm grid grid-cols-2 gap-2">
            <div><dt className="text-zinc-400 inline">Net capital deposited: </dt><dd className="inline">{formatSar(goal.netDepositedHalalas)}</dd></div>
            <div><dt className="text-zinc-400 inline">Current total assets: </dt><dd className="inline">{formatSar(goal.netWorthHalalas)}</dd></div>
            <div><dt className="text-zinc-400 inline">Trading days left in 2026: </dt><dd className="inline">{goal.remainingTradingDays}</dd></div>
            <div>
              <dt className="text-zinc-400 inline">Avg. daily growth still needed: </dt>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href="/portfolio" className="text-xs text-emerald-400">View portfolio &rarr;</Link>
        </div>
        {holdings.length === 0 ? (
          <p className="text-sm text-zinc-400">
            You don&apos;t hold any positions yet. <Link href="/market" className="text-emerald-400">Browse the market</Link> to place your first trade.
          </p>
        ) : (
          <ul className="divide-y divide-zinc-800">
            {holdings.slice(0, 5).map((h) => (
              <li key={h.stock.id} className="py-2 flex items-center justify-between text-sm">
                <Link href={`/market/${h.stock.ticker}`} className="hover:text-emerald-300">
