    ...
    return (
      <div className="max-w-lg mx-auto space-y-4">
        <div>
          ...
          <p className="text-sm text-zinc-500">Your Al Rajhi Capital mutual fund holdings.</p>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-6 text-sm text-zinc-500">
          ...
          product from your self-directed <Link href="/portfolio" className="text-emerald-700">stock holdings</Link> and
          your <Link href="/abian" className="text-emerald-700">Abian</Link> portfolio.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-zinc-200 rounded-lg p-4">
        ...
        <div className={`text-sm mt-1 ${account.todayGainHalalas < 0n ? "text-red-600" : "text-emerald-700"}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-zinc-200 rounded-lg p-4">
          <div className="text-xs text-zinc-500">Total cash</div>
        ...
        <div className="bg-white border border-zinc-200 rounded-lg p-4">
          ...
          <div className={`text-xl font-semibold ${gainTotal < 0n ? "text-red-600" : "text-emerald-700"}`}>
            {formatSar(gainTotal)}
          </div>
        </div>
      </div>
          ...
          <tbody>
            {account.holdings.map((h) => (
              <tr key={h.id} className="border-b border-zinc-50 last:border-0">
                ...
                <td className={`py-2 px-4 text-right ${h.gainHalalas < 0n ? "text-red-600" : "text-emerald-700"}`}>
