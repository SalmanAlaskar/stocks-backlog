...
export default function IpoSubscribeForm({ ipoId, perInvestorCapSar }: { ipoId: string; perInvestorCapSar: number }) {
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      ...
      <div>
        <label className="block text-xs text-zinc-500 mb-1">Subscription amount (SAR, max {perInvestorCapSar.toLocaleString()})</label>
        <input type="number" step="0.01" name="amount" required max={perInvestorCapSar} className="w-40 rounded border border-zinc-300 px-2 py-1.5 text-sm" />
      </div>
